...
class Mesa {
    ...
    correr() {
        document.getElementById("mostrar").innerHTML = this.nome + " é uma FODENDO MESA DE JANTAR, não pode correr (infelizmente)";
        console.log("${this.nome} é uma mesa,  felizmente ou infelizmente não pode correr");
    }
}
...
function criaObjeto() {
    ...
    if (patas === 2) {
        ...
        return new Cadeira(nome);
    } else if (patas >= 6 && patas <= 8){
        let respira = prompt("Respira?");
        if (respira === "sim" || respira === "Sim" || respira === "SIM" || respira === "s" || respira === "S") {
            return new Inseto(nome);
        }
        return new Cadeira(nome);
    } else  if( patas > 8){
