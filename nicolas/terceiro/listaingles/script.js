...
if (canvas.getContext) {
    ...
    // Detect click and redirect to Google search
    canvas.addEventListener('click', function (event) {
        var x = event.pageX;
        var y = event.pageY;

        for (var i = 0; i < words_attr.length; i++) {
            var word = words_attr[i];
            if (
                x > word.x &&
                x < word.x + word.width &&
                y > word.y - parseInt(word.font, 10) &&
                y < word.y
            ) {
                var searchQuery = word.text;
                window.open(`https://www.google.com/search?q=define+${searchQuery}`, '_blank');
                break;
            }
        }
    });
